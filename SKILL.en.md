...
Default daily entry: `./run-daily-digest.sh YYYY-MM-DD`. It uses Manual v5, pausing at raw selection, per-paper records, and per-page review only so the Agent can dispatch independent paper subagents and continue the full chain. Use `npm run digest:api -- YYYY-MM-DD` or `./run-daily-digest.sh YYYY-MM-DD --api` only when the user explicitly requests the API/LLM route. Data-only entry: `./run-full-fetch.sh` (or `node scripts/full-fetch.js` / `npm run fetch`).
...
### 2.1 Default Manual v5 Without Model APIs

Manual v5 is the default daily workflow, not an automatic fallback from an unavailable model. The API/LLM route is user-selected; every path retains its normal source, content, publication, and remote-verification gates:

1. `manual_offline` replaces only the filtering-model decision. It does not claim that the filtering API ran.
2. Manual v5 replaces only the deep-analysis model with controlled full-text editorial review. Records v3 assemble spec v5 and `full-text-evidence-v5`. `manual-analysis-record.md` requires `editorialPlan` v2 plus a SHA-bound 2,400–24,000-character `editorial.readerArticle` using its paper-specific `###` headings. It must bind questions, anchors, evidence pillars, result narratives, exact facts, and any selected image with its lead/explanation. The published order is Chinese title, English title/arXiv link, tags/score, snarky review, core summary, open resources, continuous deep reading, then final score evidence. Fixed canonical method/innovation/results/detail/limitation sections remain audit inputs and historical compatibility rendering, not the Manual v5 middle-page layout. Compatible stages are explicitly marked `executionKind=manual_attestation`; checkpoints and fingerprints prove the manual artifact contract, not that API stages executed.
3. Manual blog review replaces only the semantic review model. New batches require its v3 attestation (v2 is historical compatibility): it binds every generated file and SHA to independent notes plus eight per-file semantic checks, an independent `reviewSubagent` per page, and ordered per-image `imageFindings` covering caption, adjacent narrative, visible facts, and mobile readability; receipts use `manual_semantic`. Deterministic checks, the generation manifest, Git baseline, review protocol, Hugo gate, normal `push-blog.py`, and remote OID verification remain mandatory.

Manual work is quality- and provenance-compatible with the API route, not a claim of identical execution. Historical records/specs remain readable under their original contracts. Public articles must not expose internal evidence IDs, evidence-block numbers, scoring-source markers, stage-audit prose, prompt restatements, manual status names, template scaffolding, or repeated boilerplate left by repair templates.
...
- Automatic API canonical fixed headings must remain Chinese because `parseAnalysis` / `parse_analysis` match these anchors: `## 评分`, `## 机器摘要`, `## 标签`, `## 作者与机构`, `## 毒舌点评`, `## 核心摘要`, `## 方法概述和架构`, `## 核心创新点`, `## 实验结果`, `## 细节详述`, `## 评分理由`, `## 局限与问题`, `## 开源详情`. They are parser/audit anchors, not the Manual v5 reader layout: a valid `readerArticle` replaces the five fixed middle panels; historical pages without one retain legacy rendering.
