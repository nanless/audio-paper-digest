...
7. **Keep only decision-relevant table evidence**: preserve valid compact tables already present and do not expand them back into full source tables during revision. Use at most 2 Markdown tables per paper, each with at most 12 data rows and 8 metric columns (excluding method/dataset identifier columns). Prioritize the proposed method, strongest and representative baselines, aggregate rows, and key ablations. State the retention criterion factually before the table; never use empty placeholders such as "omitted here", "table data consistent with paper", or "see original paper".
