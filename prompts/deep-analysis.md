...
### 01.方法概述和架构
详细描述论文的核心方法及其实现架构，尽量覆盖：
- 完整输入输出流程（不限于模型，也包括系统、流程、方法论）
- 每个主要组件/步骤的名称、功能、内部结构
- 组件/步骤之间的数据流与交互方式
- 关键设计选择及其动机
- 若有多阶段或多模块，逐个解释
- 若论文中有架构图或流程图（可能有多张），**每张图都必须用 Markdown 图片语法 `![描述](图片URL)` 贴到输出中**，并结合图片内容详细说明各组件/步骤关系。**重要：你只能使用上文"论文中的图片及其URL"列表中提供的URL，禁止编造或猜测任何不存在的URL。如果该列表为空，则不要插入任何图片，只用文字描述。**
- 对专业术语做必要解释，让非该子领域读者也能理解
- 如果论文是数据集、基准测试、理论分析、综述等非模型工作，则重点描述方法框架、系统设计、评估流程或理论推导过程，而非强行套用"模型"术语
...
- `## 毒舌点评`
- `## 核心摘要`
- `## 详细分析`（含 01.方法概述和架构、02.核心创新点、03.细节详述、04.实验结果、05.评分理由）
- `## 开源详情`
