...
const USAGE = '--dry-run|--apply --crosswalk UUID --stage prepare-only|analyze [--queue new-full|reader-recovery|all] [--paper-ids arxiv:YYMM.NNNNN[,..]] [--limit pilot|N] [--concurrency 1-3]';
function parseArgs(argv) {
    const mode = argv[0]; if (!['--dry-run', '--apply'].includes(mode)) throw new Error(`Use ${USAGE}`);
    const values = {}; const paperIds = [];
    for (let i = 1; i < argv.length; i += 2) {
        if (!['--crosswalk', '--stage', '--queue', '--paper-ids', '--limit', '--concurrency'].includes(argv[i])
            || argv[i + 1] === undefined || argv[i + 1] === '') throw new Error(`Use ${USAGE}`);
        if (argv[i] === '--paper-ids') {
            paperIds.push(...argv[i + 1].split(',')); continue;
        }
        if (Object.hasOwn(values, argv[i])) throw new Error(`Use ${USAGE}`);
    ...
    if (!/^[a-f0-9]{8}-[a-f0-9]{4}-[1-8][a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/i.test(values['--crosswalk'] || '')
        ...
        || paperIds.some(id => !/^arxiv:\d{4}\.\d{4,5}$/.test(id))
