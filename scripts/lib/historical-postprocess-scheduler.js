...
function deterministicStagingRunId(crosswalkId, item, registrySha256) {
    ...
    return uuidFrom(`historical-postprocess-staging-v1\0${crosswalkId}\0${item.paperId}\0${item.runId}\0${registrySha256}\0${item.analysisSchedulerItemSha256}`);
}
function checkpointPath(root, crosswalkId, registrySha256, create = false) {
    if (typeof root !== 'string' || !path.isAbsolute(root) || !UUID_RE.test(crosswalkId || '')
        || !SHA_RE.test(registrySha256 || '')) fail('configured checkpoint root, crosswalk UUID, and registry SHA required');
    const directory = fresh.assertSafeDirectory(root, create); return path.join(directory, `${crosswalkId}.${registrySha256}.json`);
}
...
function validateCheckpoint(value, crosswalkId, registrySha256) {
    if (!value || value.contract !== CONTRACT || value.version !== VERSION || value.crosswalkId !== crosswalkId
        || value.registrySha256 !== registrySha256 || !value.items || typeof value.items !== 'object'
...
function updateCheckpoint(filename, crosswalkId, registrySha256, deps, mutate) {
    return deps.updateLocked(filename, current => {
        const prior = current ? validateCheckpoint(current, crosswalkId, registrySha256)
...
async function runHistoricalPostprocess(options, overrides = {}) {
    ...
    const filename = checkpointPath(files.historicalPostprocessSchedulerDir, options.crosswalkId, taxonomy.registrySha256, true);
    updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => value);
    const outcomes = await mapConcurrent(selected, options.concurrency, async item => {
        ...
        try {
            ...
            const record = { status: 'staged', paperId: item.paperId, analysisRunId: item.runId,
                ...
                registrySha256: taxonomy.registrySha256, ...assignmentProof, stagingRunId,
                stagingManifestSha256: staged.manifestSha256, cohortDates: item.cohortDates || [], lastError: null };
            updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => {
            ...
            const record = { status: 'failed', paperId: item.paperId, analysisRunId: item.runId,
                ...
                ...(assignmentProof || {}), stagingRunId, lastError: String(error.message).slice(0, 2000) };
            updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => {
                value.items[item.paperId] = record; return value;
            }); return record;
        }
    ...
    let checkpoint = validateCheckpoint(readJsonFile(filename, 'historical postprocess checkpoint').value,
        options.crosswalkId, taxonomy.registrySha256);
    ...
    for (const date of dates) {
        ...
        if (!ready) {
            const record = { date, status: 'blocked', reason: 'not-all-date-papers-staged' };
            updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => {
        ...
        try {
            ...
            const record = { date, status: 'staged', aggregateRunId, stagingRunIds,
                manifestSha256: aggregates[0].manifestSha256, fileSha256: outputs[0].fileSha256 };
            updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => {
                value.daily[date] = record; return value;
            }); daily.push(record);
        } catch (error) {
            const record = { date, status: 'blocked', reason: String(error.message).slice(0, 2000) };
            updateCheckpoint(filename, options.crosswalkId, taxonomy.registrySha256, deps, value => {
                value.daily[date] = record; return value;
            }); daily.push(record);
        }
    ...
    return { status: outcomes.every(item => item.status === 'staged') && daily.every(item => item.status === 'staged')
        ...
        registrySha256: taxonomy.registrySha256, processed: outcomes, daily, checkpoint: filename,
