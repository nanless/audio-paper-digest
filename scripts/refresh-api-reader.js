...
async function refreshApiReader(targetId) {
    ...
    return withPaperAnalysisLock(existing, async () => {
        ...
        console.log(`📄 只刷新读者文章: ${canonical.title || requested}`);
        const sourceDetails = await fetchArxivTextDetailed(
            canonical.arxivId || canonical.paper_id || targetId
        );
        const refreshed = await refreshApiReaderArticleFromSource(
            canonical, sourceDetails
        );
...
if (require.main === module) {
    refreshApiReader(process.argv[2]).catch(error => {
