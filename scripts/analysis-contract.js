...
const TABLE_IDENTIFIER_HEADER_RE = /(?:^|\b)(?:method|algorithm|approach|model|system|backbone|front[ -]?end|pipeline|variant|representation|embedding|feature|encoder|baseline|config(?:uration)?|dataset|corpus|benchmark|task|experiment|evaluation|test|comparison|control|boundary|slice|language|scenario|condition|setting|split|category|type|modality|version|stage|phase|step|round|epoch|decoder?|context|metric|measure)(?:\b|$)|方法|算法|方案|模型|系统|骨干|前端|流程|变体|表征|嵌入|特征|编码器|基线|配置|数据集|语料|基准|任务|实验|检验|评估|测试|比较|对照|边界|切片|语言|场景|条件|设置|划分|类别|类型|模态|版本|阶段|步骤|轮次|训练轮|解码|上下文|指标|度量/i;
