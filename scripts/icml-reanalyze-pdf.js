...
/**
 * 为已找到 PDF 的 ICML 论文重新进行基于全文的深度分析（支持双模型模式）
 */
...
const MAX_RETRIES = parseInt(process.env.ICML_ANALYZE_RETRIES || '3', 10);
...
async function analyzeWithPdf(paper, pdfText) {
    ...
    const analysis = await callModel(messages, 8000);
...
async function analyzeWithRetry(paper, pdfText) {
    ...
    console.log(`  ❌ ${paper.id} 分析失败（重试 ${MAX_RETRIES} 次）: ${lastError.message}`);
...
async function main() {
    ...
    console.log(`=== 重新分析完成 ===`);
    console.log(`成功: ${completed}, 失败: ${failedCount}`);
