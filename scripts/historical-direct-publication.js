...
const USAGE = [
    'visual-disposition --apply --plan-file ABS --mode excluded|waived --reason TEXT --output ABS.json',
    'plan --dry-run|--apply --publication-id UUID --plan-file ABS --registry-file ABS --projection-file ABS --visual-disposition ABS',
    'generate --dry-run|--apply --publication-id UUID --plan-file ABS --registry-file ABS --projection-file ABS --visual-disposition ABS',
    'review|activate|publish --dry-run|--apply --publication-id UUID [--message TEXT]',
...
function parseArgs(argv) {
    ...
    if (action === 'visual-disposition') {
        ...
        const values = parsePairs(rest, ['--plan-file', '--mode', '--reason', '--output']);
        ...
        return { action, apply: true, planFile: values['--plan-file'], dispositionMode: values['--mode'],
            reason: values['--reason'], output: values['--output'] };
    }
    ...
    const values = parsePairs(rest, stageNeedsAuthority
        ? ['--publication-id', '--plan-file', '--registry-file', '--projection-file', '--visual-disposition']
...
function main(argv = process.argv.slice(2), runtime = {}) {
    ...
    if (options.action === 'visual-disposition') {
        ...
        const value = api.buildVisualDisposition({ plan, mode: options.dispositionMode, reason: options.reason });
    ...
    const common = { outputRoot: Config.FILES.historicalDirectPublicationDir, publicationId: options.publicationId,
        blogRepo: Config.PUBLISH_CONFIG.blogRepo, remoteName: Config.PUBLISH_CONFIG.githubRemote || 'origin' };
