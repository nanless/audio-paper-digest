...
const api = require('./lib/historical-openreview-pdf-source.js');

const USAGE = '--dry-run|--apply --snapshot ABSOLUTE.json --forum-id OPENREVIEW_ID [--pdf-root ABSOLUTE_DIR] [--receipt-root ABSOLUTE_DIR]';
function parseArgs(argv) {
    ...
    if (!path.isAbsolute(values['--snapshot'] || '') || !/^[A-Za-z0-9_-]{6,128}$/.test(values['--forum-id'] || '')
        ...
        || (values['--receipt-root'] !== undefined && !path.isAbsolute(values['--receipt-root']))) throw new Error(`Use ${USAGE}`);
...
async function main(argv = process.argv.slice(2), runtime = {}) {
    requireExternalRuntime('historical-openreview-pdf-source.js');
...
if (require.main === module) {
    main().catch(error => { console.error(`[historical-openreview-pdf-source] ${error.message}`); process.exitCode = 1; });
}
