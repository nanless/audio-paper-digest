...
describe('Manual v6 shadow audit', () => {
    ...
    it('shadow 审计参数保持显式，默认 digest wiring 已切到 production v6', () => {
        ...
        assert.strictEqual(pkg.scripts['manual:shadow'], 'node manual/scripts/manual-v6-shadow.js');
        assert.strictEqual(pkg.scripts['manual:shadow:benchmark'], 'node manual/scripts/manual-shadow-benchmark.js');
        assert.strictEqual(pkg.scripts['manual:spec'], 'node manual/scripts/create-manual-analysis-spec-v6.js --production');
        assert.strictEqual(pkg.scripts['manual:v6:shadow:spec'], 'node manual/scripts/create-manual-analysis-spec-v6.js --shadow');
