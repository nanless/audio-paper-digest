...
Visual generation is a post-publication stage. First finish deep analysis for every paper, then generate, review, push, and remotely verify all blog pages (the digest index plus every paper page). Only then does `push-blog.py` create resumable image tasks: one tall Chinese-body infographic for each final-score TOP 10 paper, with stable normalized-arXiv-ID tie breaking, plus one batch digest image containing the title, hot directions, and TOP 5 ranking. Each paper poster targets roughly 220–360 Chinese characters of substantive explanation. When deep analysis selected and cached a trustworthy method overview, architecture, pipeline, or key-result figure, the task binds up to two references and their SHA values so built-in image generation can preserve real structural relationships while redrawing them in the shared visual system. Paper infographics and the digest cover use a fresh science-magazine style with warm light backgrounds, restrained low-saturation colors, flat editorial illustration, and generous whitespace; dark neon, cyberpunk HUDs, and crowded dashboard layouts are explicitly prohibited. These assets do not enter or block the already completed blog generation/review/push transaction.
