...
This project generates "Speech / Music / Audio Paper Digests," covering the complete pipeline from arXiv and HuggingFace Papers crawling through publishing Hugo blog posts, WeChat Official Account drafts, Xiaohongshu (Little Red Book) copy, and Feishu (Lark) documents. The default daily route is Manual v5: each paper is handled by an isolated subagent for selection, full-text research reading, scoring, and page review. LLM/API filtering and analysis remain an explicit user-selected route. Node-side tunable parameters and current runtime data-file paths are centralized in `scripts/config.js`; shared Python publish/maintenance paths are centralized in `scripts/path_config.py`.

Manual v5 uses `editorialPlan` v2 to turn full-text evidence into a paper-specific long-form article instead of a repeated field template. A SHA-bound 2,400–24,000-character `editorial.readerArticle` supplies the continuous deep reading with its own headings; natural-language result narratives prevent method/baseline/metric/value field strings from leaking into reader prose. A published paper page keeps Chinese title, English original title/arXiv link, tags/score, a pointed but evidence-bound snarky review, core summary, open resources, continuous deep reading, and final score evidence. The daily digest card preserves the same reader order: Chinese title + English title/link + tags/score → snarky review → core summary → open resources. Legacy records without a valid reader article retain their historical layout.
