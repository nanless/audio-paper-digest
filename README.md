...
显式 API/LLM 路线会在评分阶段生成结构化 `evidenceProfile`，再由代码按消融、评测范围、样本规模与部署测量应用可解释上限，降低“文字写得像高分但证据没有跟上”的偏差。事实与评分闭环后，独立的 `beginner-researcher-v2` 读者文章阶段以论文特有标题重组背景、方法、实验与边界；它要求 3–8 组术语组合解释、至少 2 张表格的表前问题/表后结论闭环，并由模型从真实 Figure 清单选择教学图，在正文中用唯一 marker 固定“图前导读—官方原图—图后解释”的顺序。旧 13 节 canonical 继续承担机器解析兼容，博客优先展示这篇连续长文。
