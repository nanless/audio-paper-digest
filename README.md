...
深度分析采用分阶段证据预算控制 token：主分析仍覆盖全文，超长论文按全文位置和任务关键词均衡取样；开源扫描、审校重写、评分审计、方法/表格和结构修复只接收各自相关的证据切片，不再重复发送整篇论文。实验表格只保留支撑结论的关键行列，且原文仅存在表格不再自动触发补表 LLM 调用；新分析和重分析还必须通过版本化 `bounded-v1` 硬契约，超出 2 张表、12 个数据行或 8 个指标列会在评分前进入局部结构修复。无版本标记的既有成功记录保持兼容，不会因此全量重跑。博客文本 review 默认 8000 字一块；结构化响应若只产生隐藏推理而没有最终 JSON，只做一次纯 JSON 有界恢复，不再盲目翻倍到 16000。所有预算都可在项目 `.env` 覆写并绑定恢复指纹。博客 review 通过项另存为“仓库相对路径 + 文件 SHA-256”的持久凭证：代码、脚本、文档、模型、协议、generation manifest 或博客基线变化只刷新整批发布凭证，不会重审正文未变的页面；新增或内容变化的页面才重新进入三层 review。视觉 plan/status 与 `digest:status` 默认仅打印紧凑索引，完整事实与来源健康明细仍保存在 manifest/报告 JSON。
