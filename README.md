...
发布后还生成一张批次汇总图，标题、热门方向和 TOP 10 排行榜均从已发布批次对应的审计数据确定性计算。论文长图与汇总图独立绑定数据、prompt、任务 token 和资产 SHA；重跑只补失败、缺失、损坏或指纹失效项。生成资产直接按批次日期归档到 `data/archive/<日期>/visual-summaries/` 和 `data/archive/<日期>/digest-cover/`，论文长图目录以 `01-` 至 `10-` 的排行榜名次开头，严格按最终 rank 排列而非生成完成顺序；图片不滞留在 current。它们不进入已经发布的博客清单，也不构成博客 generate/review/push 的前置门禁。
