...
- 当前新日更使用 Manual v5：records v3、spec v5、`manualDepth=full-text-evidence-v5`。每篇绑定 `researchBrief=audio-researcher-v1`、单论文隔离 subagent、显式 `stageReviews`、独立 scoring/readability reviewer、跨实验组 resultClaims、精确数量来源及逐图 select/reject/render plan。ingestion 重放 official assembler，canonical 与 Python 发布预检重新核对受控全文 manifest membership。records v2/spec v4 只作历史兼容。
