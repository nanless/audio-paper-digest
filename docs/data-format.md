...
- `referenceImages` 最多两张，只接纳深度分析已选中且本地缓存 URL、MIME、字节数和 SHA 全部匹配的论文原图；方法总览、架构、框架和流程图优先于实验图。参考图摘要进入分析/任务指纹，原图缺失、损坏或变化时只使对应论文长图失效。`cachePath` 指向待传给内置生图的本地原始字节，调用前可按 `mime` 复制为带扩展名的临时参考文件。
