...
- 若 LLM review 服务在整批审查期间不可用，可显式调用 `python3 scripts/manual-review-blog.py --date YYYY-MM-DD --attestation ATTESTATION.json` 进入 `manual_complete` 接管。attestation v2 除批次八项检查外，必须逐文件记录 path、SHA、独立 notes，并确认标题元数据、技术叙事、事实、实验比较、复现、局限、评分和图片；脚本仍执行确定性 review、generation manifest/baseHead/review protocol 绑定及 Hugo gate，若确定性层修改任何页面则拒绝复用旧声明。receipt 的逐文件模式为 `manual_semantic`，push 会重新核验逐文件 provenance。该模式不会把普通网络/配额错误自动降级为通过。
- 完全离线深度分析先用 `npm run manual:spec -- --date YYYY-MM-DD --records RECORDS.json` 组装 v3 spec；并行分片可重复传入 `--records`。每份输入必须是 `manual_analysis_records` v1 envelope，且 date、agent、reviewProtocol 一致；每篇必须提供合法八维评分、实际 audit passes、阶段审查次数、全文绑定的 `authorInfo`、至少六条覆盖五章的 evidenceLedger，并按 `prompts/manual-analysis-record.md` 写出论证型摘要、闭合方法流、缺口—机制—证据—边界创新、比较/消融实验、复现细节与双层局限。组装器要求 complete v2 全文 manifest 并验证批次、论文集合、指定版本、路径、来源身份和 SHA；`titleOverride` 只修标题空白。图片省略选择时从 manifest 安全候选中自动取最多三张；存在合格候选时显式空数组阻断，页面 chrome、资助方 logo、赞助素材和碎片图注会被过滤。缺失、模板复用、跨日期或任一指纹漂移均在原子写 spec 前阻断。
- 随后使用 `npm run manual:analyze -- --date YYYY-MM-DD --spec SPEC.json`。spec 必须逐篇绑定受控全文、当前人工写作 prompt、阶段 prompt、事实引用、两轮审计和十个阶段的 reviewed claims；命令不调用任何 LLM/API。完整 editorial 直接成为最终章节，短方法/创新字段仅供审计而不会机械前置。正文通过 `full-text-evidence-v3` 后，安全图片按稳定段落 ID 插入并再次完整校验，图注只在完整句号处收束。各兼容阶段写 `executionKind=manual_attestation`，明确表示人工覆盖而不是 API 执行；任何一篇失败都会拒绝写入 canonical。
