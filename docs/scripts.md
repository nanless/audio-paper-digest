...
- 完全离线深度分析先用 `npm run manual:spec -- --date YYYY-MM-DD --records RECORDS.json` 组装 v4 spec；并行分片可重复传入 `--records`。每份输入必须是 `manual_analysis_records` v2 envelope，且 date、agent、reviewProtocol 一致；每篇必须提供合法八维评分、实际 audit passes、阶段审查次数、全文绑定的 `authorInfo`、至少六条覆盖五章的 evidenceLedger，并按 `prompts/manual-analysis-record.md` 写出论证型摘要、闭合方法流、证据丰富实验表、复现细节与双层局限。组装器要求 complete v2 全文 manifest 并验证全部指纹。存在合格图片时必须显式选择 1–4 张并逐图绑定锚点、后续结论及专属邻文。
- 随后使用 `npm run manual:analyze -- --date YYYY-MM-DD --spec SPEC.json`。命令不调用任何 LLM/API。当前 spec v4 写入 `full-text-evidence-v4`，并强制 `experimentTables=evidence-rich-v2` 与上下文图片契约；旧 spec v3 仍可读取，但保留 v3 + `bounded-v1` 历史语义。各兼容阶段写 `executionKind=manual_attestation`；任何一篇失败都会拒绝写入 canonical。
