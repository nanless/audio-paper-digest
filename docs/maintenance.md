...
同时必须输出 Overall Recommendation：Strong Accept / Accept / Weak Accept / Reject / Strong Reject

机器摘要字段：
- `rank_bucket`（带顶会映射：Oral级 / Strong Accept级 / Accept级 / Reject级）
...
- `rank_bucket` 只能从 `前10%(Oral级) / 前25%(Strong Accept级) / 前50%(Accept级) / 后50%(Reject级)` 中选择
- `9.0-10.0`：Strong Accept — 突破性贡献，领域里程碑候选，值得 Oral/Highlight
- `7.5-8.5`：Accept — 明显优秀，有扎实创新和较强影响力，小缺陷可在 camera-ready 修正
- `5.5-7.0`：Weak Accept / Borderline — 有价值但不够突出，或有小硬伤，属于合格到良好
- `3.5-5.0`：Reject — 创新有限、实验薄弱、结论不够重要或存在明显缺陷
- `1.0-3.0`：Strong Reject — 问题严重，推导错误、实验不支持结论或写作极差，不推荐投入时间
...
5. 验证微信/小红书/飞书脚本产物，确认文案中没有因字段缺失导致的空值或格式错位
6. 确认 `## 局限与问题` 新章节在博客/社媒中正确显示（如有内容）
