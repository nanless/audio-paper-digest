...
- **Image Analysis**: first preselects candidate figures by caption/filename/order heuristics (default `imageCandidateMax=20`). Dual-model mode then downloads up to `imageMaxCount=20` images serially and sends them to the secondary model; single-model mode only stores candidate URL/manifest metadata and does not download base64 images. Defaults are 6MB raw bytes per image, 8M base64 chars per image, and 20M total base64 chars per paper. In dual-model mode, the secondary model outputs only a JSON insertion plan; code inserts selected figures, lead/explanation text, and any optional local `replacement` around a precisely matched anchor. If no usable images are available, the flow falls back to text-only analysis
