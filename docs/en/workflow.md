...
The default daily entry is `./run-daily-digest.sh YYYY-MM-DD`. Starting from fetch requires Beijing today because the fetcher binds the batch to its process start date and accepts no historical-date injection; historical batches may only resume existing production state from tasks/spec/analyze/generate/review/push/visual. The default path is production Manual v6. Blog generation reads only the standard production canonical at `data/current/deep-analysis-result.json`; it does not infer a production batch from a v5 file or a dated archive. The script coordinates the deterministic stages and deliberately stops at boundaries where the main Agent must dispatch real paper or page subagents. It never calls an image API; after publication succeeds, Codex must use built-in `image_gen` to complete the TOP 10 paper infographics and digest cover, then pass both visual status gates. The legacy automatic data-only entry remains `./run-full-fetch.sh` (or `node scripts/full-fetch.js` / `npm run fetch`) and is not the default Manual route.
...
The normal path is production Manual v6. Keyword prefiltering plus LLM filtering, staged LLM deep analysis, and ordinary API-backed blog review run only when the user explicitly requests the API/LLM path. Neither model-service failures nor a manual choice may silently change a paper or batch's provenance:
