...
- `manual-deep-analysis.js` calls no LLM/API. Every spec binds controlled full text and SHA, `manual-analysis-record.md`, stage prompt hashes, source-locatable evidence, two audit passes, and distinct reviewed claims. Manual v3 additionally requires a reader-visible argument: a progressive summary, an input-to-output method flow, gap–mechanism–evidence–boundary innovation claims, comparative/ablation and negative results, reproducibility details, and separately labelled evidence-backed versus reviewer-derived limitations. Complete editorial sections are the final prose; compact method/innovation fields remain audit inputs and are not prepended. Compatible stage records are explicitly marked `executionKind=manual_attestation`; they never claim that an LLM stage ran. Safe paper figures are inserted into actual prose instead of existing only as metadata, and captions may only be shortened at a complete sentence boundary. Canonical output is written only after the whole batch passes `full-text-evidence-v3`.
