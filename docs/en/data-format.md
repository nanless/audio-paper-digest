...
- New analyses and reanalyses write `analysisManifest.contracts.experimentTables=evidence-rich-v2` after structure repair. Besides the existing upper bounds, the marker requires an identifier field, three evidence rows, two numeric cells, metric directions, a specific pre-table comparison question, and a post-table synthesis with its evidence boundary; source-provided ablations or negative results must be represented. Manual v4 also binds `editorialQuality=reader-facing-v1`; `manualTakeover` stores source-closed `resultClaims` and their SHA, a seven-dimension `readabilityRubric` and its SHA, and `review.readerQualityVerified=true`. `npm run validate:data` and Python publication preflight enforce matching semantics. Historical `bounded-v1` and unmarked successes retain their original compatibility behavior
