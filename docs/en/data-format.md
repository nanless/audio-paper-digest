...
This manifest is created only after every blog page is remotely verified as published and the receipt binds a production-v6 generation proof. Its paper set is the final-score TOP 10 with normalized-arXiv-ID tie breaking, and it is not a blog-publication input. Shadow or legacy-v5 maintenance publications cannot create new visual tasks. Legacy v1/v2 visual manifests may still be archived through their explicit maintenance commands.
...
The unified read-only completion snapshot written by `npm run digest:status -- --date YYYY-MM-DD`. It summarizes candidate fetch, filtering-decision coverage, canonical completeness, strict blog review and remote OID verification, paper infographics, the digest cover, and their error lists. `analysis.complete` requires exact filtered-set coverage and a complete provenance chain for the selected route: `llm_api_production` for the default API workflow or records-v4/spec-v6 with `runtimeMode=production` for explicit Manual runs. Legacy and shadow analyses remain `invalid_or_legacy`. The terminal prints only compact stage counts and errors, while full diagnostics such as `fetch.sourceHealth` remain in JSON. `visuals.gateComplete` combines status, counts, canonical asset validation, archive uniqueness, and publication binding; `overallStatus` becomes `complete` only when every required stage completes.
...
- `blog-generation-manifest-YYYY-MM-DD.json`: the formal schema-v3 manifest contains the exact non-empty, unique Markdown file set, input/dependency fingerprints, base HEAD, category, per-file hashes, and the validated `publishedPapers` snapshot actually rendered to the blog. A default API batch declares `publicationMode=llm_api_production`; `llmApiBindings` binds Reader v3 article/plan/figures/authors, scoring audit/evidence, analysis/source SHA, model, and protocol, while `llmApiProduction` closes the exact paper set and binding fingerprint. An explicit Manual batch declares `publicationMode=manual_v6_production`; `manualV6Bindings` and `manualV6Production` bind its runtime, spec/records/Merkle/task/longform proof. Mixed or incomplete modes fail closed. `visualSummaryRequired` and `digestCoverRequired` remain `false`; post-publication images never enter this manifest.
...
- `blog-review-receipt-YYYY-MM-DD.json`: review stores file hashes and each file's actual pass-protocol fingerprint, then binds the current generation-manifest SHA-256, production `publicationMode`, the matching API or Manual proof fingerprint, current review protocol, publication scope, and Hugo gate. After exact Git-delta verification and push, it adds `publicationCommit`, matching live `remoteVerifiedOid`, remote identity, and Beijing-time `remoteVerifiedAt`. Visual planning accepts `postPublishVisuals=required` from either complete production mode, reopens the exact generation bytes, and binds both the publication commit and generation SHA into every task token. Single-paper, legacy-v5 maintenance, and historical generation receipts are explicitly not applicable to new visual planning.
