...
The unified read-only completion snapshot written by `npm run digest:status -- --date YYYY-MM-DD`. It summarizes candidate fetch, filtering-decision coverage, production-v6 canonical completeness, strict blog review and remote OID verification, paper infographics, the digest cover, and their error lists. `analysis.complete` requires exact filtered-set coverage plus complete v6 provenance with `runtimeMode=production`; legacy or shadow analysis is reported as `invalid_or_legacy`. The terminal prints only compact stage counts and errors, while full diagnostics such as `fetch.sourceHealth` remain in JSON. `visuals.gateComplete` is the final infographic gate combining status, counts, canonical asset validation, archive uniqueness, and production publication binding, so the terminal never infers completion from counts alone. `overallStatus` is `complete` and the command exits zero only when every required stage is complete.
...
- `blog-generation-manifest-YYYY-MM-DD.json`: the formal schema-v3 manifest contains the exact non-empty, unique Markdown file set, input/dependency fingerprints, base HEAD, category, per-file hashes, and the validated `publishedPapers` snapshot actually rendered to the blog. A production batch declares `publicationMode=manual_v6_production`. `manualV6Bindings` repeats each paper's production runtime, spec root/shard, sealed record, record/envelope file, ArtifactIndex, task-evidence, longform, and article identities. `manualV6Production` closes the paper set over one spec-v6 Merkle root, the spec-v6-to-records-v4 mapping, and the bindings fingerprint; its own fingerprint is independently stored. Any mixed v5/v6 input, missing field, different root, or fingerprint drift fails closed. `visualSummaryRequired` and `digestCoverRequired` remain `false`; post-publication images never enter this manifest.
...
- `blog-review-receipt-YYYY-MM-DD.json`: review stores file hashes and each file's actual pass-protocol fingerprint, then binds the current generation-manifest SHA-256, production `publicationMode`, production-proof fingerprint, current review protocol, publication scope, and Hugo gate. After exact Git-delta verification and push, it adds `publicationCommit`, matching live `remoteVerifiedOid`, remote identity, and Beijing-time `remoteVerifiedAt`. Visual planning accepts only `postPublishVisuals=required` on a production-v6 receipt, reopens the exact generation bytes, and binds both the publication commit and generation SHA into every task token. Single-paper, legacy-v5 maintenance, and historical generation receipts are explicitly not applicable to new visual planning.
