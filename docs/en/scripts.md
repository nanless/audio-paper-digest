...
- New analyses and reanalyses write `analysisManifest.contracts.experimentTables=bounded-v1`. Before scoring, the structure-repair gate enforces at most two experimental Markdown tables, 12 data rows and 8 metric columns per table; Node data validation and Python publication preflight enforce the same marker. Legacy successful records without the marker remain compatible. Missing-table repair only runs for an explicit `Table` / `Tbl.` / `表` citation without a Markdown table, or an illegal omission marker.
...
2. `review-blog.py` runs code, strict LLM, and multimodal review only for pages that still need it, then applies deterministic checks and the Hugo gate to the complete batch. Every pass is immediately stored in `blog-review-passes-YYYY-MM-DD.json`, keyed permanently by repository-relative path plus the SHA-256 actually read; failures remain in `blog-review-failure-YYYY-MM-DD.json`. Code, script, documentation, model, protocol, generation-manifest, or blog-base changes do not discard unchanged-page passes. Only new, byte-changed, transient-failure, or repaired content-failure pages re-enter review. Success issues a fresh batch receipt bound to the current manifest, base, protocol, and Hugo gate; review never commits or pushes. HTTP retries honor `Retry-After` first, otherwise use exponential backoff with short jitter; format and full-protocol repair retries have tighter independent budgets. If a reasoning model spends the whole budget without final JSON, the client performs one JSON-only recovery, growing the default 4000 budget to at most 8000 instead of repeatedly doubling to 16000.
