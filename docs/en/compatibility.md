...
| API Reader | Reader v3 + source v4 + author/resource identity v1 | v1/v2 and v3 missing any current source contract are read-only; legacy structured artifacts are accepted only when sealed TXT/source-manifest/parser layout replay exactly | requires article, plan, Figure, author/affiliation, resource-state, and source hashes to close; sealed artifacts are never rewritten |
...
| Review receipt | current review protocol | per-page passes are permanently keyed by relative path plus exact content SHA; manifest/model/code/protocol/Hugo changes never re-review unchanged files | reruns batch gates and rebinds generation, Git baseline, protocol, and Hugo gate |
| Visual summary | v3 TOP 10 | v1/v2 require explicit migration | binds publication commit/OID and the current visual token; modern ephemeral Figure identity emits empty references and never falls back to legacy cache |
...
| Historical direct staging/aggregate/publication | `historical-direct-*-v1` | private runtime artifacts remain auditable; per-page passes persist by path plus content SHA | complete direct projections can publish through independent review, locked activation/commit/push, and remote-OID verification; missing conference-aggregate integration still fails closed |
