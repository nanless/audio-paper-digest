...
本页只回答“怎样让流程可靠启动”。默认命令和阶段含义见[主流程](workflow.md)，错误现象见
[排错手册](troubleshooting.md)，全部文档入口见[文档导航](README.md)。Manual 使用同一个项目
`.env`，但其专属运行规则位于 [`manual/`](../manual/README.md)。

### 6.1 统一存放位置

**所有环境变量统一放在项目根目录的 `.env` 文件。**
